	...
	return Main(Class("container mx-auto mt-10 mb-20 px-4 max-w-4xl"),
		...
		Section(Class("bg-zinc-900/30 border border-zinc-800/50 rounded-2xl shadow-xl shadow-black/10 p-6 md:p-8 mb-6"),
			...
			Div(Class("text-sm text-zinc-400 space-y-1"),
				Div(g.Text(fmt.Sprintf("Uptime: %s", uptimeStr))),
				Div(Class("flex items-center gap-2"),
					g.Text("AI Normalization: "),
					g.If(aiEnabled,
						Span(Class("inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-900/50 text-emerald-400 border border-emerald-800/50"), g.Text("Enabled")),
					),
					g.If(!aiEnabled,
