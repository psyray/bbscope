...
func DocsContent() g.Node {
	return Main(Class("container mx-auto mt-10 mb-20 px-4 max-w-3xl"),
		...
		Section(Class("bg-zinc-900/30 border border-zinc-800/50 rounded-2xl shadow-xl shadow-black/10 p-6 md:p-8 mb-6"),
			...
			P(Class("text-zinc-400 leading-relaxed"), g.Text("This platform aggregates public bug bounty program scopes from HackerOne, Bugcrowd, Intigriti, YesWeHack, Yogosha and BugBounty.ch. All data is fetched automatically using the bbscope CLI tool.")),
		),
